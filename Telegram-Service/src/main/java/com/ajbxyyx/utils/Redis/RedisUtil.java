...
import com.ajbxyyx.utils.JsonUtil;
...
public class RedisUtil {
    ...
    private static String maxScanCount = "100";
    ...
    public static <T> List<T> mget(String keyPrefix,Class<T> t) {
        String luaScript = """
                local cursor = '0';
                local keys = {};
                repeat
                    local result = redis.call('SCAN', cursor, KEYS[1], ARGV[1]);
                    cursor = result[1];
                    
                    local resultKeys = result[2];
                    
                    for i = 1, #resultKeys do
                        table.insert(keys,resultKeys[i]);
                    end
                ...
                """;
        List<String> keys = Arrays.asList(keyPrefix);
        List<String> args = Arrays.asList(maxScanCount);
        ...
        List<T> result = stringRedisTemplate.execute(redisScript, keys, args);
        return result;
    }
    ...
    /**
     * 对key的value -1 如果大于0且存在
     * @param key
     * @param amount
     * @return
     */
    public static Boolean decrIfgt0(String key,Integer amount) {
        ...
        String luaScript = """
                ...
                if(v and v - {cnt} >= 0) then
                    redis.call('set', KEYS[1],v-{cnt});
                    return '1';
                    end;
                return '0';
                
                """.replace("{cnt}",String.valueOf(amount));
        redisScript.setScriptText(luaScript);
        redisScript.setResultType(String.class);                             //KEYS[1]
        String res = stringRedisTemplate.execute(redisScript, Collections.singletonList( key ) );
        Boolean ok = res.equals("1") ? true : false;
        return ok;
    }

    public static Boolean viewerAdd(String key,Integer amount) {
        DefaultRedisScript<String> redisScript = new DefaultRedisScript<>();

        String luaScript = """
                local key = redis.call('hexists' , KEYS[1] ,ARGV[1]);
                ...
                if(key == 1) then
                    ...
                    redis.call('hset', KEYS[1], ARGV[1],v + ARGV[2]);
                    return '1';
                    end;
                return '0';
        ...
        redisScript.setScriptText(luaScript);
        redisScript.setResultType(String.class);                             //KEYS[1]
        String res = stringRedisTemplate.execute(redisScript, Collections.singletonList( key ) ,"Viewer",String.valueOf(amount) );
        Boolean ok = res.equals("1") ? true : false;
        return ok;
    }
     ...
     * @return
     */
    public static Long delPrefix(String redisKeyPrefix) {
...
//        List<String> args = Arrays.asList(redisKeyPrefix);

        DefaultRedisScript<Long> redisScript = new DefaultRedisScript<>();
        redisScript.setScriptText(luaScript);
        redisScript.setResultType(Long.class);
        Long execute = stringRedisTemplate.execute(redisScript, keys, redisKeyPrefix);
        return execute;
    }


    public static Boolean hdecrIftg0(String key,String field,String decValue) {

        String luaScript = """
                local current = tonumber(redis.call('HGET', KEYS[1], ARGV[1])) or 0
                if (current - ARGV[2]) > 0 then
        ...
        String result = stringRedisTemplate.execute(redisScript, keys, field,decValue);
